...
@Component({
    selector: 'student-sticker-report',
    templateUrl: './student-sticker-report.component.html',
    providers: [RdpzsdReportResource]
})
export class StudentStickerReportComponent {
    ...
    exportReportExcel() {
        return this.resource.exportReportExcel(this.filter)
            .subscribe((blob: Blob) => {
                saveAs(blob, 'СтикериСтуденти.xlsx');
            });
    }
